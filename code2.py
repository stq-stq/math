...
# 建立SARIMA模型
model = SARIMAX(data.iloc[:,4], order=(1, 1, 1), seasonal_order=(1, 1, 1, 12))
results = model.fit()
...
# 比较模型的AIC
model_non_seasonal = SARIMAX(data.iloc[:,4], order=(1, 1, 1), seasonal_order=(0, 0, 0, 0))
results_non_seasonal = model_non_seasonal.fit()

print(col1_name+f'Seasonal Model AIC: {results.aic}')
print(col1_name+f'Non-Seasonal Model AIC: {results_non_seasonal.aic}')
# In[1]:
seasonal_diff = data.iloc[:,4] - data.iloc[:,4].shift(12)
seasonal_diff.dropna(inplace=True)

result = adfuller(seasonal_diff)
print(col2_name+'ADF Statistic:', result[0])
print(col2_name+'p-value:', result[1])
...
print(col2_name+f'Seasonal Model AIC: {results.aic}')
print(col2_name+f'Non-Seasonal Model AIC: {results_non_seasonal.aic}')
...
plt.show()
# In[1]:
# 生成[测试数据](https://geek.csdn.net/educolumn/0ae403dfc3a2481ebc47f05903364887?spm=1055.2569.3001.10083)
x = df.iloc[:,0].values
y = df.iloc[:,2].values

# 小波变换
w = pywt.Wavelet('db4') # 选择小波[函数](https://geek.csdn.net/educolumn/ba94496e6cfa8630df5d047358ad9719?dp_token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpZCI6NDQ0MDg2MiwiZXhwIjoxNzA3MzcxOTM4LCJpYXQiOjE3MDY3NjcxMzgsInVzZXJuYW1lIjoid2VpeGluXzY4NjQ1NjQ1In0.RrTYEnMNYPC7AQdoij4SBb0kKEgHoyvF-bZOG2eGQvc&spm=1055.2569.3001.10083)
